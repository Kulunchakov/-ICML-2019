...
template <typename T>
__global__ void kernel_add_exp(const int n, T* data, T* data2, T b);

template <>
__global__ void kernel_add_exp(const int n, double* data, double* data2, double b) {
   CUDA_KERNEL_LOOP(index, n) {
      data2[index] = exp(data[index]+b);
   }
};

template <>
__global__ void kernel_add_exp(const int n, float* data, float* data2, float b) {
   CUDA_KERNEL_LOOP(index, n) {
      data2[index] = expf(data[index]+b);
   }
};
...
template <typename T>
__global__ void kernel_mult(const int n, T* data, T* data2, T* data3) {
   CUDA_KERNEL_LOOP(index, n) {
      data3[index] = (data[index]) * (data2[index]);
   }
}

template <typename T>
__global__ void kernel_div(const int n, T* data, T* data2) {
   CUDA_KERNEL_LOOP(index, n) {
      data[index] = (data[index]) / (data2[index]);
   }
}
...
template <typename T>
__global__ void kernel_custom6(const int n, T* data, T* data2, T* data3) {
   CUDA_KERNEL_LOOP(index, n) {
      data3[index]=fmax(T(1.0)-data[index]*data2[index],T(0));
      data3[index]=data3[index]*data3[index];
   }
};

template <typename T>
__global__ void kernel_custom7(const int n, T* data, T* data2, T* data3) {
   CUDA_KERNEL_LOOP(index, n) {
      data3[index]=data[index]-data2[index];
      data3[index]=T(0.5)*data3[index]*data3[index];
   }
};
...
template <typename T>
void cuda_add_exp(const int n, T* data, T* data2, T b,cudaStream_t U) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_add_exp<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS, 0, U>>>(current_size, data+i, data2+i, b);
   }
};


template <typename T>
void cuda_sqr(const int n, T* data,cudaStream_t U) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_sqr<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS, 0 , U>>>(current_size, data+i);
   }
};

template <typename T>
void cuda_sqr(const int n, T* datain, T* dataout, cudaStream_t U) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_sqr<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS, 0, U>>>(current_size, datain+i,dataout+i);
   }
};

template <typename T>
void cuda_mult(const int n, T* data, T* data2, T* data3) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_mult<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS>>>(current_size, data+i, data2+i, data3+i);
   }
};
...
template <typename T>
void cuda_div(const int n, T* data, T* data2) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_div<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS>>>(current_size, data+i, data2+i);
   }
};
...
template <typename T>
void cuda_inv_thrs(const int n, T* data, T* out, T thrs,cudaStream_t U) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_inv_thrs<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS,0,U>>>(current_size, data+i,out+i,thrs);
   }
};
...
template <typename T>
void cuda_sqrt(const int n, T* data, cudaStream_t U) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_sqrt<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS, 0, U>>>(current_size, data+i);
   }
};
...
template <typename T>
void cuda_custom6(const int n, T* data, T* data2, T* data3) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_custom6<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS>>>(current_size, data+i, data2+i,data3+i);
   }
};

template <typename T>
void cuda_custom7(const int n, T* data, T* data2, T* data3) {
   for (int i=0; i<n; i+=MAX_SIZE) {
      const int current_size=MIN(n-i,MAX_SIZE);
      kernel_custom7<T><<<GET_BLOCKS(current_size), CUDA_NUM_THREADS>>>(current_size, data+i, data2+i,data3+i);
   }
};
...
template void cuda_add_exp (int, double*, double*, double,cudaStream_t);
template void cuda_add_exp (int, float*, float*, float,cudaStream_t);

template void cuda_inv (int, double*);
template void cuda_inv (int, float*);

template void cuda_inv_thrs (int, double*,double*, double,cudaStream_t);
template void cuda_inv_thrs (int, float*,float*, float,cudaStream_t);
...
template void cuda_sqrt (int, double*,cudaStream_t);
template void cuda_sqrt (int, float*,cudaStream_t);

template void cuda_sqr (int, double*,cudaStream_t);
template void cuda_sqr (int, float*,cudaStream_t);
template void cuda_sqr (int, double*, double*,cudaStream_t);
template void cuda_sqr (int, float*, float*,cudaStream_t);
...
template void cuda_custom6 (int, double*,double*,double*);
template void cuda_custom6 (int, float*,float*,float*);

template void cuda_custom7 (int, double*,double*,double*);
template void cuda_custom7 (int, float*,float*,float*);

template void cuda_mult (int, double*, double*);
template void cuda_mult (int, float*, float*);

template void cuda_mult (int, double*, double*, double*);
template void cuda_mult (int, float*, float*, float*);

template void cuda_div (int, double*, double*);
template void cuda_div (int, float*, float*);
